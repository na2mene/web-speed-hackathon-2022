...
function useTodayRacesWithAnimation(races) {
  ...
  useEffect(() => {
    const isRacesUpdate =
      ...
      ].reduce((a, b) => a.filter(c => !b.includes(c))).length !== 0;
...
export const Top = () => {
  ...
  const { data: raceData } = useFetch(`/api/races?since=${moment(date).unix()}&until=${moment(date).add(1, 'days').unix()}`, jsonFetcher);
